...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {mobileOpen && (
        <div className="fixed inset-0 bg-black/40 z-20 lg:hidden" onClick={closeMobile} />
      )}
      ...
      <div className={[
        ...
        <div className="h-full"><Sidebar onClose={closeMobile} /></div>
      </div>

      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile header */}
        <header className="lg:hidden sticky top-0 z-10 flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-200">
          <button onClick={openMobile} className="p-2 rounded-lg hover:bg-gray-100 text-gray-600" aria-label="Buka menu">
            <Menu className="w-5 h-5" />
          </button>
          {logo ? (
            <img src={logo} alt="Logo" className="h-7 w-auto max-w-[120px] object-contain" />
          ) : (
